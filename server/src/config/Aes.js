...
export default class Aes {
  ...
  constructor(aesKeyString, initializationVectorString = undefined) {
    ...
    this.aesKey = Buffer.alloc(this.aesKeyByteLength);
    aesKeyBufOriginal.copy(this.aesKey, 0, 0, this.aesKeyByteLength);

    // Initialization Vector.
    const ivString = initializationVectorString || '';
    this.initializationVector = Buffer.alloc(16);
    this.initializationVector.write(ivString);
  }

  encrypt(unencrypted) {
    const cipher = crypto.createCipheriv('aes-256-cbc', this.aesKey, this.initializationVector);
  ...
  decrypt(encrypted) {
    ...
    const cipher = crypto.createDecipheriv('aes-256-cbc', this.aesKey, this.initializationVector);
